...
服务默认优先识别质量（`large-v3` + `beam_size=5`），在 CPU 或低端 GPU 上会偏慢。可以通过以下环境变量做速度优化：
...
- `WHISPER_BEAM_SIZE`：解码束宽，默认 `5`。追求速度可调到 `1`~`2`。
- `WHISPER_VAD_FILTER`：默认 `true`。若输入语音较干净且追求速度，可设为 `false`。
- `PRELOAD_MODEL_ON_STARTUP`：设为 `true` 可减少首请求冷启动延迟。
- `WS_PARTIAL_MIN_BYTES`：WebSocket 增量识别最小字节增量，默认 `32768`，避免每个小 chunk 都触发全量重识别。
- `WS_PARTIAL_MIN_INTERVAL_MS`：WebSocket 增量识别最小触发间隔，默认 `700` 毫秒，降低高频转码与解码开销。
